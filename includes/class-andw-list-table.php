...
if (!class_exists('ANDW_List_Table') && class_exists('WP_List_Table')):
class ANDW_List_Table extends WP_List_Table {
    private $requesters;
    private $workers;
    ...
    public function prepare_items() {
        ...
        $allowed_orderby = ['date', 'title', 'andw_date'];
        ...
        if (!empty($_GET['status'])) {
            ...
            $meta_query[] = ['key'=>'_andw_status','value'=>$status_filter,'compare'=>'=','type'=>'CHAR'];
        }
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Display-only filter/sort; no state change
        if (!empty($_GET['requester'])) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Display-only filter/sort; no state change
            $requester_filter = sanitize_text_field( wp_unslash( $_GET['requester'] ) );
            $meta_query[] = ['key'=>'_andw_requester','value'=>$requester_filter,'compare'=>'=','type'=>'CHAR'];
        }
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Display-only filter/sort; no state change
        if (!empty($_GET['worker'])) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Display-only filter/sort; no state change
            $worker_filter = sanitize_text_field( wp_unslash( $_GET['worker'] ) );
            $meta_query[] = ['key'=>'_andw_worker','value'=>$worker_filter,'compare'=>'=','type'=>'CHAR'];
        }
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Display-only filter/sort; no state change
        if (!empty($_GET['target_type'])) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Display-only filter/sort; no state change
            $target_type_filter = sanitize_key( wp_unslash( $_GET['target_type'] ) );
            $meta_query[] = ['key'=>'_andw_target_type','value'=>$target_type_filter,'compare'=>'=','type'=>'CHAR'];
        }
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Display-only filter/sort; no state change
        ...
        if ($from) $meta_query[] = ['key'=>'_andw_work_date','value'=>$from,'compare'=>'>=','type'=>'CHAR'];
        if ($to)   $meta_query[] = ['key'=>'_andw_work_date','value'=>$to,'compare'=>'<=','type'=>'CHAR'];

        $orderby_arg = 'date';
        if ('andw_date' === $orderby) {
        ...
        foreach ($q->posts as $p) {
            ...
            $items[] = [
                ...
                'status'        => get_post_meta($pid, '_andw_status', true) ?: '依頼',
                'requester'     => get_post_meta($pid, '_andw_requester', true),
                'worker'        => get_post_meta($pid, '_andw_worker', true),
                'date'          => get_post_meta($pid, '_andw_work_date', true),
                'target_type'   => get_post_meta($pid, '_andw_target_type', true),
                'target_id'     => get_post_meta($pid, '_andw_target_id', true),
                'target_label'  => get_post_meta($pid, '_andw_target_label', true), // 旧データ互換用
                'work_title'    => get_post_field('post_title', $pid) ?: get_post_meta($pid, '_andw_work_title', true),
                'work_content'  => get_post_field('post_content', $pid) ?: get_post_meta($pid, '_andw_work_content', true),
