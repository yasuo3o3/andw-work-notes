...
if ( ! class_exists( 'ANDW_List_Table' ) && class_exists( 'WP_List_Table' ) ) :
	class ANDW_List_Table extends WP_List_Table {
		...
		public function __construct( $args = array() ) {
			...
			$this->requesters = $args['requesters'] ?? array();
			$this->workers    = $args['workers'] ?? array();
		}
		...
		protected function get_sortable_columns() {
			return array(
				'andw_date' => array( 'andw_date', false ),
		...
		protected function extra_tablenav( $which ) {
			...
			$allowed_status = array( '', '依頼', '対応中', '完了' );
			...
			$allowed_target_type = array( '', 'post', 'site', 'other' );
			if ( ! in_array( $tt, $allowed_target_type, true ) ) {
				$tt = ''; }
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Display-only filter/sort; no state change
		...
		public function get_views() {
			...
			$views    = array();
			$statuses = array(
				''    => __( 'すべて', 'andw-work-notes' ),
				'完了'  => __( '完了', 'andw-work-notes' ),
				'対応中' => __( '対応中', 'andw-work-notes' ),
				'依頼'  => __( '依頼', 'andw-work-notes' ),
			);
		...
		private function status_counts() {
			...
			$allowed_statuses = array( '依頼', '対応中', '完了' );
			$default_status   = '依頼';
			...
			if ( false === $status_counts ) {
				...
				$status_counts = (array) $wpdb->get_results(
					$wpdb->prepare(
		...
		public function prepare_items() {
			...
			$allowed_orderby = array( 'date', 'title', 'andw_date' );
